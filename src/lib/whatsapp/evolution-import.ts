...
import { resolveInboundPhone } from './providers/jid';
...
export async function importEvolutionHistory(
  ...
  while (!contactsDone) {
    ...
    try {
      ...
      result.errors.push(
        `findContacts failed at offset ${contactsOffset}: ${message}`
      );
      break;
    }
...
async function importContactHistory(
  ...
  // Classify the JID: only PN identities yield a phone. LID entries
  // without a PN pair, malformed user parts, etc. are skipped rather
  // than stored as fake phone numbers.
  const identity = resolveInboundPhone({ remoteJid: contact.remoteJid });
  if (!identity.phone) {
    outcome.skippedContact = true;
    return outcome;
  }
  ...
  while (!messagesDone) {
    ...
    for (const rawMsg of messages) {
      ...
      const inserted = await insertHistoricalMessage(db, conversationId, event);
      if (inserted) {
