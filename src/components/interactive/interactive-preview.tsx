 ...
 * customer taps them on their own device). User-provided copy renders
 * verbatim; only the empty-state fallbacks are localized so it can be
 * dropped into the composer, the automation builder, and the
 * quick-replies manager without namespace coupling.
 */
    ...
    >
      <div className="px-3 py-2">
        ...
        <p className="whitespace-pre-wrap break-words text-sm">
          {payload.body || (
            <span className="text-muted-foreground">{t("fallbackBody")}</span>
          )}
        </p>
        {payload.footer ? (
      ...
      {payload.kind === "buttons" ? (
        <div className="flex flex-col border-t border-border">
          {payload.buttons.map((b, i) => (
            ...
            >
              ...
              <span className="truncate">{b.title || t("fallbackButton")}</span>
            </button>
          ))}
        </div>
      ) : (
        ...
        >
          ...
          <span className="truncate">{payload.button_label || t("fallbackMenu")}</span>
        </button>
      )}
    </div>
  );
