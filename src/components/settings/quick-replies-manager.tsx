...
export function QuickRepliesManager() {
  ...
  const save = useCallback(async () => {
    if (!draft) return;
    if (!draft.title.trim()) {
      toast.error(tError("quick_reply_title_required"));
      return;
    }
    const payload =
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(
          tError(errorCodeOf(data) ?? (draft.id ? "quick_reply_update_failed" : "quick_reply_create_failed")),
        );
        return;
      }
      toast.success(tError(draft.id ? "quick_reply_updated" : "quick_reply_created"));
      setDraft(null);
      await load();
    } catch {
  ...
  const remove = useCallback(
    async (id: string) => {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        toast.error(tError(errorCodeOf(data) ?? "quick_reply_delete_failed"));
        return;
      }
      toast.success(tError("quick_reply_deleted"));
      await load();
    },
    [load, t, tError],
  );
